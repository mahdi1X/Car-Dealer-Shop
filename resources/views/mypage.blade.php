...
@section('content')
    ...
    @foreach ($cars as $car)
        <div class="col-sm-4 mb-4">
            <div class="car-card h-100 d-flex flex-column" >
                ...
                <!-- Car Image -->
                <img src="{{ asset('storage/' . $car->image) }}" alt="Car Image" style="height: 50%; object-fit: cover;">
    ...
    <a href="{{ route('cars.edit', $car->id) }}" class="btn btn-sm btn-primary btn-custom-sm">Edit</a>
    <a href="{{ route('cars.show', ['car' => $car->id]) }}" class="btn btn-sm btn-primary btn-custom-sm">View</a>
    <form action="{{ route('cars.destroy', $car->id) }}" method="POST" onsubmit="return confirm('Are you sure?');" style="margin: 0;">
        @csrf
        @method('DELETE')
